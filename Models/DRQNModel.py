...
from Models.EpisodalReplayBuffer import EpisodalReplayBuffer
...
class DRQNModel(StrategyRLModel):
    __slots__ = [
        "__policy_network",
        "__target_network",
        "__current_h",
    ...
    def __init__(
        ...
        if policy_network is None:
            self.__policy_network = RecurrentQN(
                UnifiedRaceState.size(), 128, len(SimpleRaceStrategy)
            ).to(self._device)
        else:
            self.__policy_network = policy_network.to(self._device)

        if target_network is None:
            self.__target_network = RecurrentQN(
                UnifiedRaceState.size(), 128, len(SimpleRaceStrategy)
            ).to(self._device)
        else:
            self.__target_network = target_network.to(self._device)

        self.__current_h = None
    ...
    def train(
        ...
        episodal_replay_buffer = EpisodalReplayBuffer(replay_buffer_size)
        ...
        while i_episode <= end_episode:
            h = self.__policy_network.get_init_hidden_state(1, self._device)

            self._logger.log(
                message=f"{self.name} | Episode {i_episode} of {end_episode} | Epsilon {epsilon:.2f}",
                verbose=verbose,
            )
            ...
            while not state.terminal:
                action, greedy_action, h = (
                    utils.simple_strategy_epsilon_greedy_recurrent(
                        epsilon=epsilon,
                ...
                if len(episodal_replay_buffer) >= 1:
                    episode = episodal_replay_buffer.sample()
                    ...
                    (
                        ...
                        torch.stack(x).unsqueeze(0).to(self._device)
                        for x in zip(*episode)
                    )
            ...
            if exception is not None and exception.simulator_at_fault:
                ...
                episodal_replay_buffer.delete_current_episode()
                continue
            else:
                episodal_replay_buffer.store_current_episode()
    ...
    def __loss(
        ...
        (
            q_out,
            _,
        ) = self.__policy_network(x=states, device=self._device)
        q_a = q_out.gather(2, actions)

        (
            q_target,
            _,
        ) = self.__target_network(x=next_states, device=self._device)
        q_target_max = q_target.max(2).values
    ...
    def explain_feature_importance(
        ...
        actions: list[SimpleRaceStrategy],
        show_plot: bool = False,
    ) -> dict:
        return self._explain_feature_importance_recurrent(
            states,
            actions,
    ...
    def predict(
        ...
        Returns:
            SimpleRaceStrategy: The predicted strategy
        """
        # Reset current hidden and cell states
        if self.__current_h is None:
            self.reset_h()
        h = self.__current_h

        # Forward pass through network
        _, greedy_action, new_h = utils.simple_strategy_epsilon_greedy_recurrent(
            epsilon=0.0,
            policy=self.__policy_network,
            hidden_state=h,
            state=state,
            filter_invalid_actions=True,
            device="cuda" if torch.cuda.is_available() else "cpu",
        )

        # Update current hidden states
        self.__current_h = new_h
    ...
    def predict_q_values(
        ...
        device = "cuda" if torch.cuda.is_available() else "cpu"
        with torch.no_grad():
            q_values, new_h = self.__policy_network(
                x=state.to_tensor().unsqueeze(0).unsqueeze(0).to(device),
                h=h,
                device=device,
            )
    ...
    def reset_h(self) -> None:
        self.__current_h = self.__policy_network.get_init_hidden_state()
    ...
    def load_model(
        ...
        model = DRQNModel(
            ...
            name=checkpoint.get("name", "DRQNModel"),
